...
dep 'configured.nginx', :nginx_prefix do
  nginx_prefix.default!('/etc/nginx') # This is required because nginx.src might be cached.
...
dep 'nginx.src', :nginx_prefix, :version, :upload_module_version do
  nginx_prefix.default!("/etc/nginx")
...
dep 'unicorn-server available', :app_name, :port, :app_type do  
  ...
  app_name.ask("What is the name of application located at /etc")
  port.ask("What port do you want to choose for your application")
  met? {    
    Babushka::Renderable.new("/etc/nginx/sites-available/#{app_name}").from?(dependency.load_path.parent / "nginx/site.erb")
  }
  meet {    
    render_erb "nginx/site.erb", :to => "/etc/nginx/sites-available/#{app_name}", :sudo => true    
  }
...
dep 'unicorn-server', :app_name, :port, :app_type do
  ...
  met? {
    "/etc/nginx/sites-enabled/#{app_name}".p.exists?    
  }
  meet {    
    shell("ln -f -s /etc/nginx/sites-available/#{app_name} /etc/nginx/sites-enabled/")
